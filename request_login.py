import requests
from models import Login, create_login, get_login
...
async def login():
    ...
    for login in await get_login():
        if login:
            ...
            if len(r.cookies) == 1:
                wrong_logins.append(f'Login: {login.login} parol 🔐: {login.password}\n')
            else:
                await create_login(login=login.login,password=login.password)
                correct_logins.append(f'Login: {login.login} parol 🔐: {login.password}\n')
                l +=1
...
async def login_main(data,tg_id):
    ...
    for login in data:
            ...
            if len(r.cookies) == 1:
                wrong_logins.append(f'Login: {login.split(':')[0]} parol 🔐: {login.split(":")[1]}')
            else:
                await create_login(login=login.split(':')[0],password=login.split(':')[1])
                correct_logins.append(f'Login: {login.split(':')[0]} parol 🔐: {login.split(":")[1]}')
                l +=1
